...
#Refactor sin traducir
def generateSubsets(n: int , m: int, strategy: StrategySubsets = StrategySubsets.EXACT):
    print("He metido esta nueva funcion 117")
    print(f"Strategy: {strategy.value}, Type: {type(strategy)}")
    all_subsets = [set() for _ in range(n+1)]
    dict_pairs = {}
    ...
    for predictor in range(n):
        for size in range(n):
            ...
            candidatos_sin = [conjunto for conjunto in all_subsets[size] if predictor not in conjunto]
            candidatos_con = [(tuple(sorted(conjunto + (predictor,)))) for conjunto in candidatos_sin]
            ...
            interseccion = []
            for i in range(len(candidatos_sin)):
                if candidatos_con[i] in all_subsets[size+1]:
                    suma +=1
                    interseccion.append(candidatos_sin[i])
            ...
            while len(candidatos_sin) < m_k:
                ...
                if subset_sin not in all_subsets[size]:
                    all_subsets[size].add(subset_sin)
                    candidatos_sin.append(subset_sin)
                    candidatos_con.append(subset_con)        
                
                if subset_con not in all_subsets[size+1]:
                    all_subsets[size+1].add(subset_con)
                    
            candidatos_con = candidatos_con[:m_k]
            
            for conjunto in candidatos_con:
                if conjunto not in all_subsets[size+1]:
                    all_subsets[size+1].add(conjunto)
    ...
    #aplanar todos_conjuntos
    all_subsets = [list(conjuntos) for conjuntos in all_subsets]
    # hacer solo un conjunto de todos los conjuntos
    all_subsets = [conjunto for conjuntos in all_subsets for conjunto in conjuntos]
    #hacer una lista de conjuntos
    all_subsets = [list(conjunto) for conjunto in all_subsets]

    return dict_pairs, all_subsets
